...
describe('Test the checkout page addres section', () => {
   test('should display the input fields', async () => {
      ...
      screen.findByRole('textbox', { name: /Vezetéknév/i })
      ...
      screen.getByRole('textbox', { name: /Házszám/i })
...
describe('Test the user events with the input fields', () => {
   test('should display any error messages', async () => {
      ...
      expect(
         await screen.findByText(/Csak magyar vezetékes\/vonalas telefonszám formátum lehetséges/i)
      ).toBeInTheDocument()
...
describe('Test the user input', () => {
   test('should display a different button when the user inputs are correct', async () => {
      ...
      userEvent.type(screen.getByRole('textbox', { name: /Házszám/i }), '7')
      ...
      await waitForElementToBeRemoved(() =>
         screen.queryByText(/A számlázási adatok sikeresen rögzítésre kerültek!/i)
      )
   })
})
